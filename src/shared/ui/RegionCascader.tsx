...
export function RegionCascaderBase({
  ...
  return (
    <div className='w-full px-[10px] flex flex-col items-center relative'>
      ...
      <section
        className={`w-[327px] absolute top-[50px] z-[1] ${
          showCascader ? ' scale-y-100 ' : 'scale-y-0'
      ...
      >
        <div
          className={`w-[110px] h-[180px] p-2 border-r-[1px] bg-white border-gray-100 ${
            focusedOptionIndex === -1 && 'shadow-grid'
          } rounded-[10px] z-[2] absolute flex flex-col justify-start items-center gap-[6px]`}
        >
          {options.map((option, index) => (
            <div
              className={`w-[102px] h-[34px] rounded-[6px] flex items-center relative transition-colors cursor-pointer
              ${option?.disabled == true && 'opacity-50 cursor-not-allowed'}
              ${
                focusedOptionIndex === index
                  ? 'bg-wooco_blue-primary-light text-white'
                  : 'text-gray-700'
              }
            `}
